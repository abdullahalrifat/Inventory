    ...
    if(orderId) {
        ...
        $.ajax({
            ...
            success:function(response) {
                ...
                var mywindow = window.open('', 'Stock Management System', 'height=1200,width=600');
                mywindow.document.write('<html><head><title>Order Invoice New</title>');
                mywindow.document.write("<p><img src='logo.png' /> </p>");
...
function addRow() {
    ...
    $.ajax({
        ...
        error: function (jqXHR, textStatus, errorThrown) {
            alert("jqXHR : "+jqXHR+" textStatus: "+textStatus+" Error: " + errorThrown);
        },
        success:function(data) {
           // alert(data);
            ...
            for (var key in datas) {
                tr += '<option value="'+ datas[key].val1+'">'+datas[key].val2+'</option>';
                alert("problem");
            }
...
function subAmount() {
    ...
    // vat
    var vat = 0 ; //(Number($("#subTotal").val())/100) * 13;
    vat = vat.toFixed(2);
    $("#vat").val(vat);
    $("#vatValue").val(vat);
